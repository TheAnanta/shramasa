...
export const addProductToCart: any = async (
  req: Request,
  res: Response
): Promise<Response> => {
  ...
  try {
    ...
    return res.status(500).json({
      error:
        "Internal Server Error. Couldn't add product to cart. " + error.message,
    });
  }
};

export const modifyCart: any = async (
  req: Request,
  res: Response
): Promise<Response> => {
  ...
  try {
    ...
    if (product!.stock[variant] < quantity) {
      return res.status(402).json({
        error:
          "Quantity not available. Try " + product!.stock + " items or less",
      });
    }
    const userCurrentCart = doesCartExistForUser[0];
    const newCartItems: JsonValue[] =
      quantity > 0
        ? [
...
export const fetchUserCart: any = async (
  req: Request,
  res: Response
): Promise<Response> => {
  ...
  if (!userId) {
    return res
      .status(400)
      .json({ error: "Provide a userId to fetch the cart for." });
  }
  try {
...
export const removeInvalidProductsFromCart: any = async (
  req: Request,
  res: Response
): Promise<any> => {
  const { userId } = req.body;
